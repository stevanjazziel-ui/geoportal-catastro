...
ZIP_PATH = BASE_DIR / "SALUD_CATEGORIZADO.zip"
...
OUTPUT_EQUIPAMIENTOS = DATA_DIR / "riobamba_salud_categorizada.geojson"
OUTPUT_EQUIPAMIENTOS_STATS = DATA_DIR / "riobamba_salud_categorizada_stats.json"
OUTPUT_ISOCRONAS = DATA_DIR / "riobamba_isocronas_salud_categorizada.geojson"
OUTPUT_ISOCRONAS_STATS = DATA_DIR / "riobamba_isocronas_salud_categorizada_stats.json"
EXTRACT_DIR = DATA_DIR / "_tmp_salud_categorizada"
...
def extract_source_records():
    ...
    if shp_path is None:
        raise FileNotFoundError("No se encontro un shapefile dentro del ZIP de salud.")
    ...
    try:
        ...
        for index, shape_record in enumerate(reader.iterShapeRecords(), start=1):
            ...
            categoria = normalize_text(record.get("CATEGORIA") or record.get("categoria")).upper()
...
def ensure_cache_covers_sources(records, osm_payload):
    ...
    for record in records:
        ...
        if not (min_lon <= point.x <= max_lon and min_lat <= point.y <= max_lat):
            raise RuntimeError(
                "La red OSM cacheada no cubre todos los equipamientos de salud. "
...
def main():
    ...
    output_stats = {
        ...
        "observacion": (
            "Se generan isocronas de salud solo para BARRIAL (400 m) y ZONAL (1000 m). "
    ...
    print("Listo.")
    print(f"Equipamientos salud: {OUTPUT_EQUIPAMIENTOS}")
    print(f"Stats salud:         {OUTPUT_EQUIPAMIENTOS_STATS}")
    print(f"Isocronas salud:     {OUTPUT_ISOCRONAS}")
    print(f"Stats iso salud:     {OUTPUT_ISOCRONAS_STATS}")
