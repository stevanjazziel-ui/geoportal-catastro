(()=>{
  ...
  function renderRecords(){
    ...
    else $('records').innerHTML=`<div class="table-wrap"><table><thead><tr><th>Dirección</th><th>Intervención</th><th>Territorio</th><th>Monto específico</th><th>Consulta</th></tr></thead><tbody>${rows.map(r=>{const b=model.budgetMap.get(r.budgetId);return `<tr><td>${esc(shortArea(r.area))}<small>${esc(data.periods[r.period])}</small></td><td class="table-title">${esc(r.description)}<small>${esc(r.indicator||'Sin indicador registrado')}</small></td><td>${esc(visibleCodes(r).length>3?`${visibleCodes(r).length} plataformas`:visibleCodes(r).map(label).join(', ')||'Sin asignar')}</td><td class="table-amount">${esc(amountText(b))}<small>${esc(budgetCaption(b))}</small></td><td><button data-detail="${esc(r.id)}" aria-label="Abrir ficha de ${esc(shortArea(r.area))}">Ver ficha</button></td></tr>`;}).join('')}</tbody></table></div>`;
