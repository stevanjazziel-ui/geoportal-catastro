(()=>{
  ...
  const title=()=>state.territory==='GENERAL'?'Proyectos de varias plataformas':state.territory==='ALL'||state.territory==='URBAN'?'Proyectos específicos por plataforma':`Proyectos específicos en ${label(state.territory)}`;
  const shortTitle=()=>state.territory==='GENERAL'?'Varias plataformas':state.territory==='ALL'||state.territory==='URBAN'?'Proyectos específicos':label(state.territory);
  const selectionHint=()=>state.territory==='GENERAL'?'Aquí van los proyectos A–Q o vinculados a más de una plataforma urbana. No se atribuyen a una sola plataforma.':state.territory==='ALL'||state.territory==='URBAN'?'Listado de proyectos con código único de plataforma urbana.':'Estos son los proyectos cuyo código territorial coincide únicamente con esta plataforma.';
  const recordTitle=()=>state.territory==='GENERAL'?'Proyectos de varias plataformas':state.territory==='ALL'||state.territory==='URBAN'?'Proyectos específicos':`Proyectos de ${label(state.territory)}`;
  ...
  function getCounts(){
    ...
    rioObrasForMapCounts().forEach(r=>out.set(r.platform,(out.get(r.platform)||0)+1));
    return out;
  }
  function renderList(){
  ...
  function rioObrasForSelection(){
    return rioData.records.filter(r=>{
      ...
      return inTerritory&&rioObrasMatchesQuery(r);
    });
  }
  function rioObrasMatchesQuery(r){
  ...
  function rioObrasForMapCounts(){
    return rioData.records.filter(r=>model.isUrbanCode(r.platform)&&rioObrasMatchesQuery(r));
  }
  ...
  function render(){
    ...
    const totalWorks=s.count+rioRows.length;
    ...
    $('stats').innerHTML=`<div class="stat"><span class="label">Obras contabilizadas</span><strong>${num(totalWorks)}</strong><small>${state.territory==='GENERAL'?'De alcance general':'Matriz depurada + RioObras'}</small></div><div class="stat money"><span class="label">Monto matriz</span><strong>${s.specific===null?'—':usd(s.specific)}</strong><small>${state.territory==='GENERAL'?'No se atribuye por plataforma':'Solo monto específico depurado'}</small></div><div class="stat"><span class="label">Puntos RioObras</span><strong>${num(rioRows.length)}</strong><small>${num(rio.executing)} en ejecución · ${num(rio.planned)} por ejecutarse</small></div><div class="stat money"><span class="label">Monto RioObras</span><strong>${usd(rio.total)}</strong><small>${progressText(rio.avgProgress)}</small></div>`;
  ...
  function updateMap(){
    $('mapTitle').textContent=state.territory==='GENERAL'?'Varias plataformas':state.territory==='ALL'||state.territory==='URBAN'?'Riobamba':shortTitle();
